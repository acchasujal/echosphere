...
function buildDateRange(startDate?: string, endDate?: string): DateRangeFilter | undefined {
...
export type EnvironmentReportFilters = {
  departmentId?: number;
  startDate?: string;
  endDate?: string;
};

export async function getEnvironmentReport(filters: EnvironmentReportFilters = {}) {
...
export type SocialReportFilters = {
  departmentId?: number;
  employeeId?: number;
  startDate?: string;
  endDate?: string;
...
export async function getSocialReport(filters: SocialReportFilters = {}) {
  ...
  const deptParticipationCount = new Map<number, number>();
...
export type GovernanceReportFilters = {
  departmentId?: number;
  startDate?: string;
  endDate?: string;
};

export async function getGovernanceReport(filters: GovernanceReportFilters = {}) {
  const { departmentId, startDate, endDate } = filters;
...
export async function getEsgSummary(filters: EsgSummaryFilters = {}) {
...
function computeEnvironmentalScore(totalCo2: number, completedEcoChallenges: number): number {
...
function computeSocialScore(totalBadges: number, totalParticipations: number): number {
...
function computeGovernanceScore(openIssues: number, totalIssues: number): number {
