...
export async function getAcknowledgementsByEmployee(employeeId: number) {
...
export async function getAcknowledgementsByPolicy(policyId: number) {
...
export async function createAcknowledgement(employeeId: number, policyId: number) {
  const employee = await prisma.employee.findUnique({
    where: { id: employeeId },
  ...
  if (existing) {
    throw createError(
      `Employee ${employeeId} has already acknowledged policy ${policyId}.`,
      409
    );
  }
  ...
  try {
    ...
    await createNotification({
      ...
      console.error(
        `Failed to send acknowledgement notification to employee ${employeeId}:`,
        err
      );
    });
...
export async function deleteAcknowledgement(id: number) {
  try {
