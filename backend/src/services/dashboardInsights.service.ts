...
type InsightsPayload = Omit<DashboardInsights, 'generatedAt'>;

const DEFAULT_INSIGHTS: InsightsPayload = {
...
function getStatusCount(items: Array<{ status: string }>) {
...
function extractJson(text: string) {
  ...
  return JSON.parse(trimmed.slice(start, end + 1)) as Partial<InsightsPayload>;
}
...
function normalizeInsights(value: Partial<InsightsPayload>): InsightsPayload {
  return {
...
async function collectEsgData() {
  ...
  const [
    ...
    recentAudits,
    recentPolicies,
    recentPolicyAcknowledgements,
    carbonAggregate,
    participationCount,
    completedParticipationCount,
    csrParticipationCount,
    completedCsrParticipationCount,
  ] = await Promise.all([
  ...
  const highestEmissionTransaction = recentCarbonTransactions.reduce(
    (highest, transaction) => transaction.co2Amount > highest.co2Amount ? transaction : highest,
    recentCarbonTransactions[0],
  );
...
function buildPrompt(esgData: Awaited<ReturnType<typeof collectEsgData>>) {
...
async function generateGeminiInsights(prompt: string): Promise<InsightsPayload> {
...
function generateFallbackInsights(esgData: Awaited<ReturnType<typeof collectEsgData>>): InsightsPayload {
  ...
  const riskLevel: RiskLevel = overdueIssues > 0 || totalCo2 > 1000
    ? 'high'
